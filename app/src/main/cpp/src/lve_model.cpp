...
std::unique_ptr<LveModel> LveModel::createModelFromFile(
        AAssetManager *assetManager, LveDevice &device, const std::string &filepath) {
    Builder builder{};
    ...
    membuf mb((char*) buf, ((char*)buf) + len);
    std::istream in(&mb);
    builder.loadModel(in);
...
void LveModel::Builder::loadModel(std::istream &filestream) {
